...
6. **Deploy** (unless `--no-deploy`):
   - SCPs the rbf to `${MISTER_DIR}/Oric_<YYYYMMDD_HHMMSS>_<sha>.rbf`
     (timestamped + short git SHA, so old builds don't get clobbered).
   - Also copies it to `${MISTER_DIR}/Oric.rbf` (stable "latest" name)
     so the MiSTer menu always sees something at the same path.

`MISTER_DIR` is `/media/fat/_Aoric` — note the leading underscore-A,
which forces the entry to the top of the MiSTer cores list.
...
Edit the `MISTER_HOST` and `MISTER_DIR` vars at the top of the script:
...
- **Compile passes but the MiSTer doesn't see the new core** — check
  that `Oric.rbf` actually got refreshed (the script SSHs `cp` after
  scp; verify with `ssh root@<host> ls -la /media/fat/_Aoric`).
